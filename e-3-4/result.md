...
## After 1. optimization

The run commands were combined and unnecessary files and dependencies deleted.

### Size

```
e-3-4_frontend                                                       latest    099f6644f0e9   2 minutes ago    239MB
e-3-4_backend                                                        latest    911b8222018c   17 minutes ago   440MB
```
...
```
IMAGE          CREATED          CREATED BY                                      SIZE      COMMENT
911b8222018c   19 minutes ago   CMD ["/go/server"]                              0B        buildkit.dockerfile.v0
<missing>      19 minutes ago   ENV PORT=3001                                   0B        buildkit.dockerfile.v0
<missing>      19 minutes ago   ENV REQUEST_ORIGIN=http://localhost:3000        0B        buildkit.dockerfile.v0
<missing>      19 minutes ago   USER appuser                                    0B        buildkit.dockerfile.v0
<missing>      19 minutes ago   RUN /bin/sh -c apk --no-cache add git   && g…   144MB     buildkit.dockerfile.v0
<missing>      7 weeks ago      /bin/sh -c #(nop) WORKDIR /go                   0B
<missing>      7 weeks ago      /bin/sh -c mkdir -p "$GOPATH/src" "$GOPATH/b…   0B
<missing>      7 weeks ago      /bin/sh -c #(nop)  ENV PATH=/go/bin:/usr/loc…   0B
<missing>      7 weeks ago      /bin/sh -c #(nop)  ENV GOPATH=/go               0B
<missing>      7 weeks ago      /bin/sh -c set -eux;  apk add --no-cache --v…   290MB
<missing>      7 weeks ago      /bin/sh -c #(nop)  ENV GOLANG_VERSION=1.16.15   0B
<missing>      4 months ago     /bin/sh -c #(nop)  ENV PATH=/usr/local/go/bi…   0B
<missing>      4 months ago     /bin/sh -c [ ! -e /etc/nsswitch.conf ] && ec…   17B
<missing>      4 months ago     /bin/sh -c apk add --no-cache ca-certificates   514kB
<missing>      4 months ago     /bin/sh -c #(nop)  CMD ["/bin/sh"]              0B
...
```
IMAGE          CREATED         CREATED BY                                      SIZE      COMMENT
099f6644f0e9   3 minutes ago   CMD ["serve" "-s" "-p" "3000" "/build"]         0B        buildkit.dockerfile.v0
<missing>      3 minutes ago   USER appuser                                    0B        buildkit.dockerfile.v0
<missing>      3 minutes ago   ENV REACT_APP_BACKEND_URL=http://localhost/a…   0B        buildkit.dockerfile.v0
<missing>      3 minutes ago   ENV NODE_ENV=production                         0B        buildkit.dockerfile.v0
<missing>      3 minutes ago   RUN /bin/sh -c apk --no-cache add git   && g…   129MB     buildkit.dockerfile.v0
<missing>      2 weeks ago     /bin/sh -c #(nop)  CMD ["node"]                 0B
<missing>      2 weeks ago     /bin/sh -c #(nop)  ENTRYPOINT ["docker-entry…   0B
<missing>      2 weeks ago     /bin/sh -c #(nop) COPY file:4d192565a7220e13…   388B
<missing>      2 weeks ago     /bin/sh -c apk add --no-cache --virtual .bui…   7.84MB
